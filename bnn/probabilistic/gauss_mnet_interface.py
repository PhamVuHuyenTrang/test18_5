...
from probabilistic.gauss_mlp import GaussianMLP, GaussianMLPFlipout
...
class GaussianBNNWrapper(nn.Module, MainNetInterface):
    ...
    #self.mnet.forward(X_test, weights = prior_mnet_weight)
...
mnet_interface.MainNetInterface.hyper_shapes_learned`
        ...
        if (isinstance(self._mnet, GaussianMLP) or \
                isinstance(self._mnet, GaussianMLPFlipout)) \
                and not disable_lrt:
