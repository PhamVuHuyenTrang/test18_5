...
class GaussianMLPFlipout(MLP):
    r"""We adapt the GaussianMLP class to use the Flipout estimator instead of
    ...
    def forward(self, x, mean, rho, logvar_enc=False, mean_only=False,
        ...
        This is achieved by using the flipout estimator.
        ...
        for l in range(len(w_means)):
            ...
            if mean_only:
                ...
                # use Flipout estimator
                output = gamma

                if rand_state is None:
                    if self.has_bias:
                        bias = torch.sqrt(sigma2_hat) * \
                            torch.randn(mu_hat.shape, device=mu_hat.device)
                    else:
                        bias = None
                    delta_weight = torch.sqrt(sigma2) * torch.randn(mu.shape, device=mu.device)
                    sign_input = hidden.clone().uniform_(-1, 1).sign()
                    sign_output = output.clone().uniform_(-1, 1).sign()
                    perturbed_output = F.linear(
                        hidden * sign_input, delta_weight, bias) * sign_output
                else:
                    if self.has_bias:
                        bias = torch.sqrt(sigma2_hat) * \
                            torch.normal(torch.zeros_like(mu_hat), 1.,
                                         generator=rand_state)
                    else:
                        bias = None
                    delta_weight = torch.sqrt(sigma2) * \
                                   torch.normal(torch.zeros_like(mu), 1.,
                                                generator=rand_state)
                    sign_input = torch.bernoulli(torch.ones_like(hidden) * 0.5,
                                                 generator=rand_state) * 2 - 1
                    sign_output = torch.bernoulli(torch.ones_like(output) * 0.5,
                                                  generator=rand_state) * 2 - 1
                    perturbed_output = F.linear(
                        hidden * sign_input, delta_weight, bias) * sign_output


                hidden = output + perturbed_output
